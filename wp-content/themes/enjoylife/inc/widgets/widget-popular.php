 ...
 * @since      1.0.0
 */
class EnjoyLife_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoylife-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'enjoylife' )
		);

		// Create the widget.
		parent::__construct(
			'enjoylife-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'enjoylife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'enjoylife_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoylife_most_commented_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoylife_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'enjoylife' ),
			'limit'     => 5,
			'show_date' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title', 'enjoylife' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'enjoylife' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_date' ) ); ?>">
				<?php esc_html_e( 'Display post date?', 'enjoylife' ); ?>
			</label>
		</p>
