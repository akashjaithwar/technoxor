 ...
 * @since      1.0.0
 */
class EnjoyLife_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoylife-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'enjoylife' )
		);

		// Create the widget.
		parent::__construct(
			'enjoylife-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'enjoylife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'enjoylife_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoylife_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoylife_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'enjoylife' ),
			'limit'     => 5,
			'show_date' => true
		);
